    ...
    with open(image_list_file, 'r') as imf:
        with open(label_file, 'r') as laf:
            ...
            for line in imf:
                ...
                labels = laf.readline().strip().split()
                label,angle = int(labels[0]), float(labels[1])
                max_label = max(max_label, label)
                imgs.append((img_path, label, angle))
    assert(total_num == len(imgs))
    assert(label_num == max_label+1)
    return imgs, max_label


class MS1MV2(data.Dataset):
    ...
    def __getitem__(self, index):
        path, target, angle = self.imgs[index]
